...
public class MarkovController {
	...
	public void parse(String markovLog) {
		...
		if (!chainMap.containsKey(markovID)) {
			...
			HashMap<String, Object> lastNode = (HashMap<String, Object>) chainMap.get(markovID);
			String lastNodeStr = (String) lastNode.get("node");
			if (!lastNodeStr.equals(node)) {
				return;
			}
			int i = 0;
			while (!nodesArray[i].equals(lastNodeStr)) {
				i++;
			}
	 ...
	 * 
	 * @param log
	 *            log indicando la ID del ataque a borrar
	 */
